...
interface Props {
  ...
  defaultValues: {
    ...
    comisiones: { descripcion: string; monto: number; moneda: string }[];
  };
}

export function PagoEditForm({ tipo, pagoId, relacionadoNombre, cuentas, defaultValues }: Props) {
  const router = useRouter();
  const [comisiones, setComisiones] = useState<Comision[]>(defaultValues.comisiones);
  ...
  const neto = (montoActual || 0) - totalFees;

  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-5 max-w-lg">
      <div className="rounded-lg border border-neutral-200 bg-neutral-50 px-4 py-3 text-sm text-neutral-600">
        {tipo === "cliente" ? "Cliente" : "Proveedor"}: <span className="font-medium text-neutral-900">{relacionadoNombre}</span>
      </div>
      ...
      <div className="space-y-1.5">
        <Label>Cuenta</Label>
        <Select value={cuentaId} onValueChange={(v: string | null) => { if (v) setValue("cuentaId", v); }}>
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div className="space-y-1.5">
          <Label>Estado</Label>
          <Select value={watch("estado")} onValueChange={(v: string | null) => { if (v) setValue("estado", v as FormData["estado"]); }}>
            <SelectTrigger><SelectValue /></SelectTrigger>
            <SelectContent>
      ...
      <div className="space-y-3 rounded-lg border border-neutral-200 p-4">
        ...
        {comisiones.length > 0 && (
          <div className="pt-2 border-t border-neutral-100 space-y-1 text-sm">
            <div className="flex justify-between text-neutral-500">
              <span>Total fees</span>
              <span>− {moneda} {totalFees.toFixed(2)}</span>
            </div>
      ...
      <div className="flex gap-3 pt-2">
        <Button type="submit" disabled={isSubmitting} className="cursor-pointer min-w-[130px]">
          {isSubmitting ? <><Loader2 size={14} className="mr-1.5 animate-spin" />Guardando...</> : "Guardar cambios"}
        </Button>
